...
using Cache = std::map<int, std::map<std::string, std::string>>;

template <size_t rank>
std::string compute2(const std::string& code, unsigned char curr, const std::string& test, Cache &cache) {
    if (cache.contains(rank) and cache[rank].contains(test)) return cache[rank][test];
    if (code.size() == 0) {
        if constexpr (rank == 0)
            return test;
        else
            return compute2<rank - 1>(test, 'A', "", cache);
    }
   
    std::string best(1000, '#');
    for (auto& s : keypad2.at(curr).at(code[0])) {
       auto sub = compute2<rank>(code.substr(1, std::string::npos), code[0], test + s, cache);
       if (sub.size() < best.size()) best = sub;
    }
    cache[rank][test] = best;
    return best;
}

std::string compute1(const std::string& code, unsigned char curr, const std::string& test, Cache& cache) {
    if (code.size() == 0) {
       return compute2<1>(test, 'A', "", cache);
    }
   
    std::string best(1000, '#');
    for (auto& s : digicode2.at(curr).at(code[0])) {
       auto sub = compute1(code.substr(1, std::string::npos), code[0], test + s, cache);
       if (sub.size() < best.size()) best = sub;
    }
    return best;
}
...
int main() {
  ...
  for (const auto &code: codes) {
      fmt::print("Code {}: ", code);
      auto cache = Cache{};
      auto best = compute1(code, 'A', "", cache);
      fmt::print("best {}\n", best);
